...
if (-not (Test-LghsAdministrator)) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($self)) {
        $argLine = '-NoProfile -ExecutionPolicy Bypass -File "{0}" -SkipUpdate' -f $self
        Start-Process powershell.exe -Verb RunAs -ArgumentList $argLine | Out-Null
        return
    }
    throw 'LGHS Imager requires Administrator privileges.'
}
...
$HelperPath = Join-Path $PSScriptRoot 'LGHS-StockBootstrap-v2.ps1'
...
if (-not (Test-Path $HelperPath)) { throw "Bootstrap helper not found: $HelperPath" }
...
function Find-ImagerBackend {
    ...
    throw 'Raspberry Pi Imager backend was not found. Install the official Raspberry Pi Imager and reopen LGHS Imager.'
}
...
function Get-TargetId {
    ...
    if ($script:Mode -eq 'local') { return 'LOCAL' }
    $format = 'D{0}' -f [int]$Config.batch.padding
    return ('{0}{1}' -f $Config.batch.studentPrefix, $script:StudentNumber.ToString($format))
}
...
function Refresh-Drives {
    ...
    foreach ($disk in Get-RemovableDisks) {
        $item = New-Object System.Windows.Controls.ComboBoxItem
        $item.Content = "Disk $($disk.Number)  |  $($disk.FriendlyName)  |  $(Format-Bytes $disk.Size)"
        ...
        foreach ($partition in @(Get-Partition -DiskNumber $DiskNumber -ErrorAction SilentlyContinue)) {
            ...
            if ($volume -and $volume.FileSystem -eq 'FAT32') {
                ...
                if (-not $partition.DriveLetter) {
                    ...
                    Set-Partition -DiskNumber $DiskNumber -PartitionNumber $partition.PartitionNumber -NewDriveLetter $letter -ErrorAction Stop
                    $temporary = $true
                } else {
                    $letter = [string]$partition.DriveLetter
                }
                return [pscustomobject]@{ Root="${letter}:\"; Partition=$partition.PartitionNumber; Temporary=$temporary }
            }
        }
...
function Show-BatchRangeConfiguration {
    [xml]$x = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="Student deployment range" Width="520" Height="360" ResizeMode="NoResize" WindowStartupLocation="CenterOwner" Background="#0F1115" FontFamily="Segoe UI">
...
<Grid Grid.Row="5" Margin="0,20,0,0"><Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><Button x:Name="CancelButton" Grid.Column="1" Content="Cancel" MinWidth="90" Margin="0,0,8,0"/><Button x:Name="ContinueButton" Grid.Column="2" Content="Start deployment" MinWidth="140" Padding="16,9" Background="#2E6DB4" Foreground="White"/></Grid></Grid></Window>
'@
    $reader = New-Object System.Xml.XmlNodeReader $x
    $dialog = [Windows.Markup.XamlReader]::Load($reader)
    $dialog.Owner = $script:Window
    $startBox = $dialog.FindName('StartBox')
    $endBox = $dialog.FindName('EndBox')
    $preview = $dialog.FindName('Preview')
    $validation = $dialog.FindName('ValidationText')
    $go = $dialog.FindName('ContinueButton')
    $cancel = $dialog.FindName('CancelButton')
    $startBox.Text = [string]$script:BatchStart
    $endBox.Text = [string]$script:BatchEnd
    $refresh = {
        $a = 0; $b = 0
        if ([int]::TryParse($startBox.Text,[ref]$a) -and [int]::TryParse($endBox.Text,[ref]$b) -and $a -ge 1 -and $b -ge $a) {
            $count = $b - $a + 1
            $suffix = if ($count -ne 1) { 's' } else { '' }
            $preview.Text = "CS-$($a.ToString('D2')) through CS-$($b.ToString('D2'))  |  $count Pi$suffix"
        } else { $preview.Text = 'Enter a valid range.' }
    }
    $startBox.Add_TextChanged($refresh); $endBox.Add_TextChanged($refresh); & $refresh
    $cancel.Add_Click({ $dialog.DialogResult = $false; $dialog.Close() })
    $go.Add_Click({
        $a = 0; $b = 0; $validation.Text = ''
        if (-not [int]::TryParse($startBox.Text,[ref]$a) -or -not [int]::TryParse($endBox.Text,[ref]$b)) { $validation.Text='Use whole numbers.'; return }
        if ($a -lt 1 -or $b -lt $a -or $b -gt 999) { $validation.Text='Use a range from 1 through 999, with End >= Start.'; return }
...
function Show-DeviceConfiguration([string]$Role,[string]$DeviceId) {
    ...
    $user=[string]$Config.accounts.user; $admin=[string]$Config.accounts.admin; $root=[string]$Config.accounts.root
    $wifi = Get-LghsLocalWifiConfig
    $wifiText = if ([string]::IsNullOrWhiteSpace($wifi.Ssid)) { 'No bootstrap Wi-Fi is saved. The Pi will provision locally and finish LGHS when a network becomes available.' } else { "Bootstrap Wi-Fi saved locally: $($wifi.Ssid)" }
    [xml]$x = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="Configure device" Width="540" Height="590" ResizeMode="NoResize" WindowStartupLocation="CenterOwner" Background="#0F1115" FontFamily="Segoe UI">
...
<TextBlock x:Name="WifiStatus" Grid.Row="8" Margin="0,18,0,0" Foreground="#9CC6FF" FontSize="11" TextWrapping="Wrap"/>
<TextBlock Grid.Row="9" Margin="0,12,0,0" Foreground="#717C89" FontSize="11" TextWrapping="Wrap" Text="Passwords are verified before the card is released and are never written to LGHS Imager logs."/>
<Grid Grid.Row="10" Margin="0,22,0,0"><Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><TextBlock x:Name="Validation" Grid.Column="0" Foreground="#E88B8B"/><Button x:Name="Cancel" Grid.Column="1" Content="Cancel" MinWidth="88" Margin="0,0,8,0"/><Button x:Name="Continue" Grid.Column="2" Content="Continue" MinWidth="100" Background="#2E6DB4" Foreground="White"/></Grid></Grid></Window>
'@
    $reader=New-Object System.Xml.XmlNodeReader $x; $dialog=[Windows.Markup.XamlReader]::Load($reader); $dialog.Owner=$script:Window
    $heading=$dialog.FindName('Heading'); $sub=$dialog.FindName('Subheading'); $ul=$dialog.FindName('UserLabel'); $al=$dialog.FindName('AdminLabel'); $rl=$dialog.FindName('RootLabel')
    $up=$dialog.FindName('UserPassword'); $ap=$dialog.FindName('AdminPassword'); $rp=$dialog.FindName('RootPassword'); $same=$dialog.FindName('SameRoot'); $wifiStatus=$dialog.FindName('WifiStatus'); $val=$dialog.FindName('Validation'); $cancel=$dialog.FindName('Cancel'); $go=$dialog.FindName('Continue')
    $heading.Text="Configure $kind - $DeviceId"; $sub.Text="Set the local credentials for $DeviceId."; $ul.Text="User Password  ($user)"; $al.Text="Admin Password  ($admin)"; $rl.Text="Root Password  ($root)"; $wifiStatus.Text=$wifiText
    $same.IsChecked=[bool]$Config.accounts.defaultRootSameAsAdmin; $rp.IsEnabled=-not $same.IsChecked
    $same.Add_Checked({$rp.IsEnabled=$false; $rp.Clear()}); $same.Add_Unchecked({$rp.IsEnabled=$true}); $cancel.Add_Click({$dialog.DialogResult=$false; $dialog.Close()})
    $go.Add_Click({
        $val.Text=''
        if ([string]::IsNullOrEmpty($up.Password)) {$val.Text='Enter User password.'; return}
        if ([string]::IsNullOrEmpty($ap.Password)) {$val.Text='Enter Admin password.'; return}
        if (-not $same.IsChecked -and [string]::IsNullOrEmpty($rp.Password)) {$val.Text='Enter Root password.'; return}
        $dialog.DialogResult=$true; $dialog.Close()
    })
    if (-not $dialog.ShowDialog()) { return $null }
    $rootValue = if ($same.IsChecked) {$ap.Password} else {$rp.Password}
...
function Set-Mode([string]$Mode) {
    ...
    else {$LocalButton.IsChecked=$true;$ImageLabel.Text=if($script:LocalImage){[IO.Path]::GetFileName($script:LocalImage)}else{'Choose a local image'};$TargetLabel.Text='LOCAL'}
}
...
function Invoke-ImagerBackend([string]$Backend,[string[]]$Arguments) {
    $psi=New-Object System.Diagnostics.ProcessStartInfo; $psi.FileName=$Backend; $psi.WorkingDirectory=Split-Path -Parent $Backend; $psi.UseShellExecute=$false; $psi.RedirectStandardOutput=$true; $psi.RedirectStandardError=$true; $psi.CreateNoWindow=$true
    ...
    $process.WaitForExit();$stdout=$outTask.Result;$stderr=$errTask.Result
    foreach($line in @($stdout-split"`r?`n")){if(-not[string]::IsNullOrWhiteSpace($line)){Append-Log $line}}
    foreach($line in @($stderr-split"`r?`n")){if(-not[string]::IsNullOrWhiteSpace($line)){Append-Log $line}}
...
function Start-Flash {
    ...
    $diskNumber=[int]$DriveBox.SelectedItem.Tag;$disk=Get-Disk -Number $diskNumber;$deviceId=Get-TargetId;$role=if($script:Mode-eq'controller'){'controller'}else{'student'};$credentials=$null;$firstRunPath=$null
    ...
    Set-Busy $true;$StatusText.Text="Writing $deviceId...";Append-Log "Image source: $($source.Description)";if($source.StockBootstrap){Append-Log 'No published LGHS image found. Using official Raspberry Pi OS arm64 with LGHS first-run bootstrap.'}

    try{
        $backend=Find-ImagerBackend;Append-Log "Backend: $backend";$cliArgs=@('--cli','--disable-eject')
        if($source.StockBootstrap){
        ...
        if($source.Sha){$cliArgs+=@('--sha256',[string]$source.Sha)}
        $cliArgs+=@([string]$source.Image,"\\.\PhysicalDrive$diskNumber")
        $result=Invoke-ImagerBackend $backend $cliArgs
        if($result.ExitCode-ne0){$detail=$result.StdErr.Trim();if([string]::IsNullOrWhiteSpace($detail)){$detail=$result.StdOut.Trim()};if($detail.Length-gt1000){$detail=$detail.Substring([Math]::Max(0,$detail.Length-1000))};throw "Raspberry Pi Imager backend exited with code $($result.ExitCode). $detail"}
        ...
        if($script:Mode-ne'local'){
            ...
            Append-Log "Provisioning staged and credential round-trip verified for $deviceId."
            if($source.StockBootstrap){Append-Log 'First boot applies accounts/passwords/SSH locally. LGHS-System installation retries automatically when network becomes available.'}
        }
        $credentials=$null;$StatusText.Text="$deviceId complete - safe to remove the card."
        ...
        $credentials=$null;Append-Log "ERROR: $($_.Exception.Message)";$StatusText.Text='Write failed.';Set-LogsVisible $true
    ...
    }finally{if($firstRunPath){Remove-Item $firstRunPath -Force -ErrorAction SilentlyContinue};Set-Busy $false}
}

[xml]$xaml=Get-Content $XamlPath -Raw;$reader=New-Object System.Xml.XmlNodeReader $xaml;$script:Window=[Windows.Markup.XamlReader]::Load($reader)
...
$StudentButton.Add_Checked({if($script:Mode-ne'student'){if(-not(Show-BatchRangeConfiguration)){Set-Mode 'controller';return};Set-Mode 'student'}})
$ControlButton.Add_Checked({if($script:Mode-ne'controller'){Set-Mode 'controller'}})
$LocalButton.Add_Checked({if($script:Mode-ne'local'){$dialog=New-Object Microsoft.Win32.OpenFileDialog;$dialog.Title='Select image';$dialog.Filter='Disk images|*.img;*.zip;*.xz;*.gz|All files|*.*';if($dialog.ShowDialog()){$script:LocalImage=$dialog.FileName;Set-Mode 'local'}else{Set-Mode 'controller'}}})
...
$script:Window.Add_ContentRendered({Refresh-Drives;Set-Mode 'controller';Append-Log 'LGHS Imager ready in Control deployment mode.';Append-Log "Target: $([string]$Config.batch.controllerHostname)";Append-Log 'Missing LGHS images use stock Raspberry Pi OS ARM64 with an injected LGHS first-run bootstrap.';try{$backend=Find-ImagerBackend;Append-Log "Raspberry Pi Imager backend found: $backend"}catch{Append-Log "Backend warning: $($_.Exception.Message)"};try{$wifi=Get-LghsLocalWifiConfig;if($wifi.Ssid){Append-Log "Local bootstrap Wi-Fi profile loaded for: $($wifi.Ssid)"}else{Append-Log 'No local bootstrap Wi-Fi profile saved; offline provisioning remains supported.'}}catch{Append-Log "Wi-Fi config warning: $($_.Exception.Message)"}})
