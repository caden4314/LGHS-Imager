...
function Get-LghsLocalWifiConfig {
    ...
    if (-not (Test-Path $path)) {
        return [pscustomobject]@{ Ssid=''; Password='' }
    }
    try {
...
function Get-LghsFleetKeyPair($Config) {
    ...
    if (-not (Test-Path $private) -or -not (Test-Path $public)) {
        ...
        if (-not $sshPath) {
            throw 'Windows OpenSSH Client is required. Install the OpenSSH Client optional feature and reopen LGHS Imager.'
        }

        Remove-Item $private,$public -Force -ErrorAction SilentlyContinue
        $quotedExe = '"' + $sshPath + '"'
        $quotedKey = '"' + $private + '"'
        $cmdLine = "$quotedExe -q -t ed25519 -N `"`" -C `"LGHS fleet controller`" -f $quotedKey"
    ...
    if ($pubText -notmatch '^ssh-ed25519\s+') {
        throw 'The LGHS fleet public key is not a valid Ed25519 public key.'
    }
...
function Test-LghsCredentialRoundTrip([string]$SecretFile, $Credentials, $Wifi) {
    ...
    foreach ($check in $checks) {
        ...
        $expected = $check[1]
        $label = $check[2]
        if (-not $values.ContainsKey($key)) { throw "Credential staging failed for $label." }
        try {
            $actual = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($values[$key]))
        } catch {
            throw "Credential encoding verification failed for $label."
        }
        if (-not [string]::Equals($actual,$expected,[StringComparison]::Ordinal)) {
            throw "Credential round-trip verification failed for $label."
        }
    }
}
...
dec(){ local v; v="$(readv "$1" "$SEC")"; [[ -n "$v" || "$1" == WIFI_SSID_B64 || "$1" == WIFI_PASSWORD_B64 ]] || return 1; printf '%s' "$v" | base64 --decode; }
...
if [[ "$ROOT_SAME_AS_ADMIN" == 1 && "$ROOT_PASSWORD" != "$ADMIN_PASSWORD" ]]; then
  echo 'Root/Admin password-link verification failed.'
  exit 1
fi
...
if id -nG lg_cs_cont 2>/dev/null | tr ' ' '\n' | grep -qx sudo; then
  gpasswd -d lg_cs_cont sudo >/dev/null 2>&1 || true
fi
...
verify_password_state(){
  ...
  echo "Password credential verified for $account."
}
verify_password_state lg_cs_cont
verify_password_state cs_admin
...
if [[ -n "$WIFI_SSID" && -n "$WIFI_PASSWORD" ]] && command -v nmcli >/dev/null 2>&1; then
  ...
  if nmcli connection add type wifi ifname wlan0 con-name 'LGHS Bootstrap WiFi' ssid "$WIFI_SSID" >/dev/null 2>&1; then
    ...
    echo "Bootstrap Wi-Fi profile configured for $WIFI_SSID."
  else
    echo "Could not create bootstrap Wi-Fi profile; continuing offline."
  fi
fi
