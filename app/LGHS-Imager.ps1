...
function Show-DeviceConfiguration([string]$role, [string]$deviceId) {
    ...
    $userName = [string]$Config.accounts.user
    $adminName = [string]$Config.accounts.admin
    $rootName = [string]$Config.accounts.root

    [xml]$dialogXaml = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Configure device" Width="520" Height="550" ResizeMode="NoResize"
  ...
  <Grid Margin="26">
    ...
    <TextBlock x:Name="Heading" Grid.Row="0" FontSize="24" FontWeight="SemiBold" Foreground="#F4F6F8"/>
    <TextBlock x:Name="Subheading" Grid.Row="1" Margin="0,6,0,22" FontSize="12" Foreground="#98A2B3" TextWrapping="Wrap"/>

    <TextBlock x:Name="UserLabel" Grid.Row="2" Foreground="#AAB4C0" FontSize="12"/>
    <PasswordBox x:Name="UserPassword" Grid.Row="3" Height="38" Margin="0,6,0,16" Padding="10,7" Background="#1D2129" Foreground="White" BorderBrush="#343B47"/>

    <TextBlock x:Name="AdminLabel" Grid.Row="4" Foreground="#AAB4C0" FontSize="12"/>
    <PasswordBox x:Name="AdminPassword" Grid.Row="5" Height="38" Margin="0,6,0,12" Padding="10,7" Background="#1D2129" Foreground="White" BorderBrush="#343B47"/>
    ...
    <StackPanel Grid.Row="7">
      <TextBlock x:Name="RootLabel" Foreground="#AAB4C0" FontSize="12"/>
      <PasswordBox x:Name="RootPassword" Height="38" Margin="0,6,0,0" Padding="10,7" Background="#1D2129" Foreground="White" BorderBrush="#343B47" IsEnabled="False"/>
    </StackPanel>

    <TextBlock Grid.Row="8" Margin="0,18,0,0" Foreground="#717C89" FontSize="11" TextWrapping="Wrap"
               Text="Passwords are not saved on this Windows PC. Provisioning credentials are written to the SD card for first boot and must be consumed and removed by LGHS System."/>

    <Grid Grid.Row="9" Margin="0,22,0,0">
      <Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions>
      <TextBlock x:Name="ValidationText" Grid.Column="0" Foreground="#E88B8B" VerticalAlignment="Center" FontSize="11"/>
      <Button x:Name="CancelButton" Grid.Column="1" Content="Cancel" MinWidth="88" Padding="14,8" Margin="0,0,8,0" Background="#252A33" Foreground="White" BorderBrush="#343B47"/>
      <Button x:Name="ContinueButton" Grid.Column="2" Content="Continue" MinWidth="100" Padding="14,8" Background="#2E6DB4" Foreground="White" BorderBrush="#4D8BD1"/>
    </Grid>
  </Grid>
</Window>
'@
    ...
    $heading = $dialog.FindName('Heading')
    $subheading = $dialog.FindName('Subheading')
    $userLabel = $dialog.FindName('UserLabel')
    $adminLabel = $dialog.FindName('AdminLabel')
    $rootLabel = $dialog.FindName('RootLabel')
    $userPassword = $dialog.FindName('UserPassword')
    $adminPassword = $dialog.FindName('AdminPassword')
    $rootPassword = $dialog.FindName('RootPassword')
    $sameRoot = $dialog.FindName('SameRoot')
    ...
    $heading.Text = "Configure $kind - $deviceId"
    $subheading.Text = "Set the three local account passwords for $deviceId before this SD card is written."
    $userLabel.Text = "User Password  ($userName)"
    $adminLabel.Text = "Admin Password  ($adminName)"
    $rootLabel.Text = "Root Password  ($rootName)"
    $sameRoot.IsChecked = [bool]$Config.accounts.defaultRootSameAsAdmin
    $rootPassword.IsEnabled = -not $sameRoot.IsChecked

    $sameRoot.Add_Checked({ $rootPassword.IsEnabled = $false; $rootPassword.Clear() })
    $sameRoot.Add_Unchecked({ $rootPassword.IsEnabled = $true; $rootPassword.Focus() })
    $cancelButton.Add_Click({ $dialog.DialogResult = $false; $dialog.Close() })
    $continueButton.Add_Click({
        $validationText.Text = ''
        if ([string]::IsNullOrEmpty($userPassword.Password)) {
            $validationText.Text = 'Enter the User password.'
            $userPassword.Focus(); return
        }
        if ([string]::IsNullOrEmpty($adminPassword.Password)) {
            $validationText.Text = 'Enter the Admin password.'
            $adminPassword.Focus(); return
        }
        if (-not $sameRoot.IsChecked -and [string]::IsNullOrEmpty($rootPassword.Password)) {
            $validationText.Text = 'Enter the Root password.'
            $rootPassword.Focus(); return
        }
    ...
    $rootValue = if ($sameRoot.IsChecked) { $adminPassword.Password } else { $rootPassword.Password }
    return [pscustomobject]@{
        ...
        RootName = $rootName
        RootPassword = $rootValue
        RootSameAsAdmin = [bool]$sameRoot.IsChecked
    }
}
    ...
    do {
        ...
        foreach ($p in $parts) {
            ...
            if ($vol -and $vol.FileSystem -eq 'FAT32') {
                ...
                ) | Set-Content -Encoding ascii $publicPath

                $secretPath = "${letter}:\lghs-provision-secrets.conf"
                @(
                    'ENCODING=base64-utf8'
                    "USER_PASSWORD_B64=$(ConvertTo-Base64Utf8 $credentials.UserPassword)"
                    "ADMIN_PASSWORD_B64=$(ConvertTo-Base64Utf8 $credentials.AdminPassword)"
                    "ROOT_PASSWORD_B64=$(ConvertTo-Base64Utf8 $credentials.RootPassword)"
                ) | Set-Content -Encoding ascii $secretPath
...
function Set-Mode([string]$mode) {
    ...
    if ($mode -eq 'student') {
        ...
        $TargetLabel.Text = Get-TargetId
        $ImageLabel.Text = if ($script:LocalImage) { [IO.Path]::GetFileName($script:LocalImage) } else { 'Choose a local image' }
    }
}
...
function Start-Flash {
    ...
    $credentials = Show-DeviceConfiguration $role $deviceId
    if (-not $credentials) {
        Append-Log "Configuration for $deviceId was cancelled."
        return
    }

    $confirm = [System.Windows.MessageBox]::Show(
        "ERASE Disk $diskNumber ($($disk.FriendlyName), $(Format-Bytes $disk.Size)) and write $deviceId?",
        "Confirm $deviceId write",
    ...
    Append-Log "Starting $role image for $deviceId on Disk $diskNumber. Password values are not logged."
    ...
    try {
        ...
        $StatusText.Text = 'Writing device configuration...'
        Write-ProvisionFiles $diskNumber $deviceId $role $credentials
        $credentials = $null
        Append-Log "Provisioning data written for $deviceId."
        ...
        if ($script:Mode -eq 'student' -and $Config.batch.autoAdvanceAfterSuccessfulWrite) {
            if ($script:StudentNumber -lt [int]$Config.batch.lastDevice) {
                ...
                Set-Mode 'student'
                Append-Log "Batch advanced to $(Get-TargetId). The next Pi will ask for new passwords."
            } else {
...
foreach ($name in @('StudentButton','ControlButton','LocalButton','ImageLabel','TargetLabel','DriveBox','RefreshButton','StatusText','Progress','LogBox','WriteButton','DetailsButton')) {
...
$DetailsButton.Add_Click({
    if ($LogBox.Visibility -eq [System.Windows.Visibility]::Collapsed) {
        $LogBox.Visibility = [System.Windows.Visibility]::Visible
        $DetailsButton.Content = 'Hide details'
    } else {
        $LogBox.Visibility = [System.Windows.Visibility]::Collapsed
...
$script:Window.Add_ContentRendered({
    Refresh-Drives
    Set-Mode 'student'
    Append-Log 'LGHS Imager ready. Each Pi will request User, Admin, and Root passwords before writing.'
})
